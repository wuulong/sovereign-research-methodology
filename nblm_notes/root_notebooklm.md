...
## 🗓️ 版本演進歷史 (Version History)

本專案依照發展階段進行了版本定錨，以便於學術討論與自證追溯：

*   **`v0.1` (2026/05/14 - 第二代大腦)**：資料庫大腦雛形期。此時 SQLite 僅有單一資料表，導入 ID-Prefix 標準編碼，建立資料定錨與基礎知識分層。
*   **`v0.1.1` (2026/05/26 - 第四代大腦)**：十一表 Schema 戰役。引進 `empirical_evidences` 實體舉證表，正式開發 MCI（手稿成熟度）與 MPM（元自證成熟度）指標計量，並引入紅軍自審 `'VULNERABLE'` 合併阻斷鎖 (Verdict Lock) 防線。
*   **`v0.2` (2026/06/06 - 第五代大腦)**：去中心化獨立開源發表版。將 methodology、manuscripts、data、scripts 目錄移出為獨立 Repo 並以 Git Submodule 管理，包含四大核心主權 Skill 的初步封裝與論文手稿初版。
*   **`v0.2.1` (2026/06/06 當前最新版)**：四大技能本土化釋出與方法論大合流。
    1.  於子 Repo `skills/` 底下完整釋出四大主權核心技能，並進行台灣用語本土化清洗。
    2.  方法論規格合流重組，將 02 中繼資料 Schema 與 03 關係本體規格合流併入 02 系統規格手冊。
    3.  重建 NotebookLM 封包，排除所有非本土的 IT 中國用語。


================================================================================
📂 FILE PATH: brain_queries.sql
================================================================================
